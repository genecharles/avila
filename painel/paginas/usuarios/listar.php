...
for($i=0; $i<$linhas; $i++){
	...
	$query2 = $pdo->query("SELECT * FROM setor where id = '$lotado'");
	$res2 = $query2->fetchAll(PDO::FETCH_ASSOC);
	if (@count($res2) > 0) {
		$setor_lotado = $res2[0]['nome'];
	} else {
		$setor_lotado = 'Sem Lotação';
	}

	$tel_whatsF = str_replace("(", "", $telefone);
	$tel_whatsF = str_replace(") ", "", $tel_whatsF);
	$tel_whatsF = str_replace("-", "", $tel_whatsF);
...
<td style="color:{$classe_ativo}" class="esc">{$setor_lotado}</td>
